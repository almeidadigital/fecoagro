-- Seed new administrative user: contabilidade@fecoagro.coop.br
-- Idempotent: only inserts if the user does not already exist
-- Syncs auth.users with public.users profile table
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'contabilidade@fecoagro.coop.br') THEN
    ...
    INSERT INTO auth.users (
      ...
      crypt('Fecoagro@123456', gen_salt('bf')),
      NOW(), NOW(), NOW(),
